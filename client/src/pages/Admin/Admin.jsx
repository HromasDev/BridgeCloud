...
const Admin = () => {
	...
	const handleRoleChange = (userId) => {
		...
		const isChanged = updatedUsers.some(
			(user, index) => user.role !== initialUsers[index].role
		)
							...
							<tbody>
								{filteredUsers.map((user) => (
									<tr key={user._id}>
										<td
											className={
												isCellModified(user, '_id') ? styles.modified : ''
											}
										>
											{user._id}
										</td>
										<td
											className={
												isCellModified(user, 'vk_id') ? styles.modified : ''
											}
										>
											{user.vk_id || '-'}
										</td>
										<td
											className={
												isCellModified(user, 'name') ? styles.modified : ''
											}
										>
											{user.name}{' '}
										</td>
										<td
											className={
												isCellModified(user, 'login') ? styles.modified : ''
											}
										>
											{user.login}
										</td>
										<td
											className={`${
												isCellModified(user, 'role') ? styles.modified : ''
											}`}
										...
										<td
											className={
												isCellModified(user, 'created_at')
													? styles.modified
													: ''
											}
										>
